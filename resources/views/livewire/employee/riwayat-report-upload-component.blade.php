<flux:modal name="upload-riwayat-{{ $selectedDate }}" class="max-w-md lg:w-lg" :dismissible="false">
    <div class="space-y-6">
        <div>
            <div class="text-center">
                <i class="ph-fill ph-footprints text-white bg-blue-600 mx-auto p-1 text-2xl rounded-xl"></i>
            </div>
            <flux:heading class="text-center mt-3" size="lg">Kirim Laporan Aktivitas</flux:heading>
            <flux:text class="mt-2">Unggah bukti aktivitas untuk tanggal {{ \Carbon\Carbon::parse($selectedDate)->format('d M Y') }}</flux:text>
        </div>
            ...
            @else
                @if ($photo)
                    <div class="text-center">
                        ...
                        <button type="button" wire:click="removePhoto" class="mt-2 text-red-600 text-sm hover:underline">
                            Hapus
                        </button>
                    </div>
                @else
                    <div class="w-full flex flex-col items-center cursor-pointer">
                        ...
                        <input x-ref="fileInput" type="file" wire:model="photo"
                        ...
                        <span class="inline-block">
                            <flux:button type="button" variant="outline" color="blue" @click.prevent="$refs.fileInput.click()" class="cursor-pointer">
                                Cari File
                            </flux:button>
                        </span>
                    </div>
